...
| `src/app/today/page.tsx` | Include auf `vehicles`; Sortierung `vehicle.name` → `startTime`; Kartentitel zeigt `"Muster Bus + Transporter"`. |
...
- Mehrfach-Zuordnung (Muster Bus + Transporter) per Skript geschrieben und in
